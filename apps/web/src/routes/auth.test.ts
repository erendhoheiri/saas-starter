...
describe("signup form schema", () => {
  it("rejects missing name", () => {
    const result = signupSchema.safeParse({ name: "", email: "test@example.com", password: "password123" })
    expect(result.success).toBe(false)
  })
  it("rejects invalid email", () => {
    const result = signupSchema.safeParse({ name: "Alice", email: "not-an-email", password: "password123" })
    expect(result.success).toBe(false)
  })
  it("accepts valid signup data", () => {
    const result = signupSchema.safeParse({ name: "Alice", email: "alice@example.com", password: "password123" })
    expect(result.success).toBe(true)
  })
})
...
describe("reset password form schema", () => {
  ...
  it("rejects mismatched passwords", () => {
    const result = resetPasswordSchema.safeParse({ newPassword: "password123", confirmPassword: "different1" })
    expect(result.success).toBe(false)
  })
  it("accepts matching valid passwords", () => {
    const result = resetPasswordSchema.safeParse({ newPassword: "password123", confirmPassword: "password123" })
    expect(result.success).toBe(true)
  })
})
