...
function OrgPage() {
  ...
  const { data: membersData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.data as {
        id: string
        name: string
        members: Array<{ id: string; userId: string; role: string; user: { name: string; email: string } }>
      } | null
    },
    enabled: !!orgId,
  })
  ...
  const updateOrgMutation = useMutation({
    mutationFn: async (data: UpdateOrgForm) => {
      ...
      if (result.error) throw new Error(result.error.message ?? "Failed to update")
      return result.data
    },
    onSuccess: () => {
  ...
  if (!orgId) {
    return (
      <div className="p-8">
        ...
        <p className="text-muted-foreground">No active organization. Switch to one using the org switcher.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleInviteSubmit(onInviteSubmit)} className="space-y-3">
              ...
              />
              {inviteErrors.email && (
                <p className="text-destructive text-sm">{inviteErrors.email.message}</p>
              )}
              <select
                {...registerInvite("role")}
              ...
              {inviteErrors.root && (
                <p className="text-destructive text-sm">{inviteErrors.root.message}</p>
              )}
              {inviteMutation.isSuccess && (
                <p className="text-green-600 text-sm">Invitation sent.</p>
              )}
              <Button type="submit" disabled={inviteSubmitting || inviteMutation.isPending}>
          ...
          <CardContent>
            <form onSubmit={handleOrgSubmit(onOrgSubmit)} className="space-y-3">
              <div>
                ...
                <Input {...registerOrg("name")} placeholder="Organization name" />
                {orgErrors.name && (
                  <p className="text-destructive text-sm mt-1">{orgErrors.name.message}</p>
                )}
              </div>
              {orgErrors.root && (
                <p className="text-destructive text-sm">{orgErrors.root.message}</p>
              )}
              {updateOrgMutation.isSuccess && (
                <p className="text-green-600 text-sm">Organization updated.</p>
              )}
              <Button type="submit" disabled={orgSubmitting || updateOrgMutation.isPending}>
      ...
      {/* Delete org dialog */}
      <Dialog open={deleteOrgOpen} onOpenChange={(open) => {
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              To confirm, type <strong>delete organization</strong> below.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleDeleteOrgSubmit(onDeleteOrgSubmit)} className="space-y-4 mt-2">
            <Input
            ...
            {deleteOrgErrors.confirm && (
              <p className="text-destructive text-sm">{deleteOrgErrors.confirm.message}</p>
            )}
            ...
            <div className="flex gap-2 justify-end">
              <Button
                type="button"
                variant="outline"
                onClick={() => { setDeleteOrgOpen(false); resetDeleteOrg() }}
              >
                Cancel
              </Button>
              <Button
