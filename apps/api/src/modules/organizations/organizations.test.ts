...
async function setActiveOrg(
  ...
  if (!res.ok)
    throw new Error(`setActiveOrg failed (${res.status}): ${await res.text()}`);
...
async function getPersonalOrgId(
  testDb: unknown,
  userId: string,
): Promise<string> {
  const { schema } = await import("@starter/db");
...
describe("POST /api/organizations — create org", () => {
  it("creates a new org and returns it", async () => {
...
describe("GET /api/organizations — list my orgs", () => {
  it("returns the personal org after sign-up", async () => {
    const { withTestDb } = await import("@starter/db/test-helpers");
    await withTestDb(async () => {
      const app = await buildTestApp();
...
describe("POST /api/organizations/invite — invite a member", () => {
  it("owner can invite a member and email is sent via console provider", async () => {
    const { withTestDb } = await import("@starter/db/test-helpers");
    await withTestDb(async () => {
...
describe("POST /api/organizations/accept-invite — accept invitation", () => {
  it("invited user can accept an invitation", async () => {
    const { withTestDb } = await import("@starter/db/test-helpers");
    await withTestDb(async () => {
      const { createDb } = await import("@starter/db");
      const app = await buildTestApp();
...
describe("POST /api/organizations/members/role — update member role", () => {
  ...
  it("member cannot change roles (403)", async () => {
    ...
    await withTestDb(async () => {
      ...
      memberCookie = await signIn(app, memberEmail, "Password1!");
      const memberId = await getUserId(testDb, memberEmail);
      const insertedRows2 = await testDb
        .insert(schema.member)
        .values({
          organizationId: orgId,
          userId: memberId,
...
describe("POST /api/organizations/members/remove — remove member", () => {
  it("owner can remove a member", async () => {
    ...
    await withTestDb(async () => {
      ...
      await signUp(app, memberEmail, "Password1!");
      const memberId = await getUserId(testDb, memberEmail);
      const insertedRows3 = await testDb
        .insert(schema.member)
        .values({
          organizationId: orgId,
          userId: memberId,
      ...
      const rows = await testDb
        ...
        .where(
          and(
            ...
            eq(schema.member.userId, ownerId),
          ),
        );
  ...
  it("member cannot remove another member (403)", async () => {
    ...
    await withTestDb(async () => {
      ...
      memberCookie = await signIn(app, memberEmail, "Password1!");
      const memberId = await getUserId(testDb, memberEmail);
      const insertedRows4 = await testDb
        .insert(schema.member)
        .values({
          organizationId: orgId,
          userId: memberId,
...
describe("POST /api/organizations/set-active — switch active org", () => {
  it("sets the active org on the session", async () => {
    const { withTestDb } = await import("@starter/db/test-helpers");
    await withTestDb(async () => {
      const { createDb } = await import("@starter/db");
      const app = await buildTestApp();
